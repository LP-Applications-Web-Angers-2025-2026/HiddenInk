...
/**
 * Lance le mode interactif du programme permettant de choisir différentes
 * options de stéganographie
 * @return 0 si succès, 1 si erreur
 */
int interact() { return runInteractiveMode(); }

/**
 * Détermine la position du bit où les données seront cachées ou extraites dans
 * une image
 * @param argc Nombre d'arguments passés en ligne de commande
 * @param argv Tableau contenant les arguments passés en ligne de commande
 * @param arg_index Position dans argv[] de l'argument optionnel spécifiant la
 * position du bit (1-8)
 * @return Position du bit (0-7), ou -1 en cas d'erreur. Par défaut retourne 0
 * (LSB)
 */
 ...
 * Cette fonction affiche la syntaxe correcte et les options qui peuvent être
 * utilisées lors de l'exécution du programme, incluant les modes "HIDE",
 * "EXTRACT", "HISTO", et "INTERACT".
 */
void print_usage() {
  cerr << "Usage: "
       << " HIDE <input_bmp> <file_to_hide> <output_bmp> <encrypt> "
          "[bit_position]"
       << endl;
  cerr << "       "
       << " EXTRACT <input_bmp> <key> [bit_position]" << endl;
  cerr << "       "
       << " HISTO <input_bmp>" << endl;
  cerr << "       "
       << " INTERACT" << endl;
}

/**
 * Lance le mode interactif permettant de choisir les différentes opérations de
 * stéganographie disponibles. Affiche un menu avec 4 options :
