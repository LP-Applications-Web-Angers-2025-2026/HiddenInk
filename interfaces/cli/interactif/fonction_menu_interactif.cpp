 ...
 * Si ce n'est pas le cas, elle le crée avec tous les répertoires parents
 * nécessaires.
 */
 ...
 * Le nom de fichier inclut la date et l'heure actuelles au format
 * "YYYYMMDD_HHMMSS". Le fichier sera créé dans le dossier "out".
...
static string generateUniqueFilename(const string &prefix, const string &ext) {
  ...
  std::stringstream ss;
  ss << "./" << prefix << "_"
     << std::put_time(std::localtime(&time), "%Y%m%d_%H%M%S") << ext;
  return ss.str();
}
 ...
 * @return L'extension du fichier en minuscule (avec le point), ou ".png" si
...
static int menuCacherTexte() {
  ...
  if (encryptChoice == 'o' || encryptChoice == 'O') {
    string rawKey = generate_key(16);
    key = to_hex(rawKey);
    cout << "Clé (hex) : " << key << "\n";
  }

  if (outPath.empty()) {
    outPath = generateUniqueFilename(
        "texte_cache", formatExt); // change le nom du texte_cacher bof
    ...
    string filename =
        (lastSlash != string::npos) ? outPath.substr(lastSlash + 1) : outPath;
    outPath = "./" + filename;
  ...
  if (formatExt == ".bmp") {
    ...
    hidePNGText(carrierPath, message, outPath, 0, key);
  }

  // Sauvegarder la clé dans un fichier si chiffrement activé
  if (!key.empty()) {
    saveKeyFile(outPath, key);
    cout << "💾 Clé sauvegardée dans le fichier .hiddenink\n";
  }
...
static int menuCacherImage() {
  ...
  if (formatExt == ".bmp") {
    // Pour BMP, utiliser bmp_convert avec l'image secrète comme "fichier à
    // cacher"
    bmpHide(carrierPath, secretPath, outPath, 0, key); // bitPos = 0 par défaut
  } else {
    // Appeler la fonction PNG dédiée
    hidePNGImage(carrierPath, secretPath, outPath, 0);
  }
...
/**
 * Gère l'interface utilisateur pour extraire des données (texte ou image) d'une
 * image. Permet de:
...
static int menuExtraire() {
  ...
  if (extractChoix == 1) {
    ...
    if (ext == ".bmp") {
      ...
      getline(cin, key);
      key = cleanPath(key);
      extractPNGText(inputPath, outputPath, key);
    }
...
/**
 * Traite le choix de l'utilisateur dans le menu principal et appelle la
