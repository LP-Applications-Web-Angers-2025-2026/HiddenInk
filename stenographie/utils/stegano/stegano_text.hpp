...
inline string getTextSignature() { return "!#TEXT#!"; }
...
inline int detectTextBitsPerChannel(unsigned char *carrier, int cw, int ch,
                                    int cc);

// Cacher un message texte dans une image
inline vector<unsigned char> hideTextInImage(unsigned char *carrier, int cw,
                                             int ch, int cc,
  ...
  // 3. Le message en binaire (chiffré si clé fournie)
  string messageToEncode = message;
  if (!key.empty()) {
    string byteKey = hex_to_key(key);
    messageToEncode = xor_encrypt(message, byteKey);
    cout << "🔒 Message chiffré avec la clé fournie.\n";
  }
  ...
  if (messageBinaire.size() > capacite) {
    ...
    cerr << "   Besoin de " << messageBinaire.size()
         << " bits, capacité: " << capacite << " bits.\n";
    cerr << "   Capacité max: " << (capacite / 8) << " caractères.\n";
    return {};
  }

  cout << "\n📊 INFORMATIONS :\n";
  cout << "   Image porteuse : " << cw << "x" << ch << " (" << cc
       << " canaux)\n";
  ...
  cout << "   Utilisation : " << fixed << setprecision(2)
       << (messageBinaire.size() * 100.0 / capacite) << "%\n";
  ...
  for (int i = 0; i < cw * ch * cc && bitIndex < messageBinaire.size(); ++i) {
    for (int b = 0; b < bitsPerChannel && bitIndex < messageBinaire.size();
         ++b) {
      encoded[i] &= ~(1 << b);
...
inline string extractTextFromImage(unsigned char *carrier, int cw, int ch,
                                   int cc, int bitsPerChannel = 0,
                                   const string &key = "") {
