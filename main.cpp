...
/*
int cw, ch, cc, sw, sh, sc, w, h, c;

std::string img1, img2, imgPath, inputPath, outPath, carrierPath,secretPath, message, outputPath;
...
if (argc > 1) {
    ...
    if (mode == "extract-text" && argc >= 3) {
        ...
        inputPath = argv[2];
        outputPath = (argc >= 4) ? argv[3] : ""; // Fichier .txt optionnel
        int bits = (argc >= 5) ? std::stoi(argv[4]) : 0; // 0 = auto
